...
class DocumentVerificationService:
    ...
    def initiate_digilocker(cls, request: DigiLockerInitiateRequest) -> DigiLockerInitiateResponse:
        return cls._sandbox_provider.initiate(request)

    @classmethod
    def verify_digilocker(cls, db: Session, request: DigiLockerVerifyRequest) -> DigiLockerVerifyResponse:
        return cls._sandbox_provider.verify(db, request)

    @classmethod
    def get_user_verification_status(cls, db: Session, user_id: Optional[str] = None) -> DocumentVerificationStatusResponse:
        """Returns status of all mandatory documents under MoSJE guidelines."""
        mandatory_types = [
            ("CASTE_CERTIFICATE", "जाति प्रमाण पत्र (Caste Certificate)"),
            ("INCOME_CERTIFICATE", "आय प्रमाण पत्र (Income Certificate)"),
            ("AADHAAR", "पहचान प्रमाण / आधार (Identity Proof - Aadhaar)"),
            ("PROJECT_REPORT", "प्रोजेक्ट रिपोर्ट / कार्य योजना (Project Proposal / DPR)")
        ]
